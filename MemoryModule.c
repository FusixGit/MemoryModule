...
HMEMORYMODULE MemoryLoadLibrary(const void *data, size_t size, bool bForceMemFree)
{
    return MemoryLoadLibraryEx(data, size, bForceMemFree, MemoryDefaultLoadLibrary, MemoryDefaultGetProcAddress, MemoryDefaultFreeLibrary, NULL);
}

bool TryFreeVirtualMemory(LPVOID Addr, unsigned int Size)
{	
	...
	while(dwCurrAddress-(DWORD)Addr < Size)
	{
		bRes = VirtualQuery((LPVOID)dwCurrAddress, &iMem, sizeof(iMem));
		if(!bRes) return false;

		if(iMem.State == MEM_FREE)
		{
			DWORD dwAllocated = iMem.RegionSize;
			if((DWORD)iMem.BaseAddress < (DWORD)Addr) dwAllocated -= ((DWORD)Addr-(DWORD)iMem.BaseAddress);

			dwCurrAddress += iMem.RegionSize;
			continue;
		}

		bRes = VirtualFree(iMem.BaseAddress, 0, MEM_RELEASE);
		if(!bRes) return false;
	}
	return true;
}

HMEMORYMODULE MemoryLoadLibraryEx(const void *data, size_t size, bool bForceMemFree,
    CustomLoadLibraryFunc loadLibrary,
    CustomGetProcAddressFunc getProcAddress,
    CustomFreeLibraryFunc freeLibrary,
    void *userdata)
{
	...
	if(bForceMemFree) TryFreeVirtualMemory((LPVOID)(old_header->OptionalHeader.ImageBase), alignedImageSize);
